...
#define LUA_LIB
#include "lua.hpp"
#define PROJECT_MODULENAME "libfoo"

extern "C" {
	int LUA_API luaopen_libfoo(lua_State *L);
}
...
int l_Foo_test(lua_State * l)
{
  int argc = lua_gettop(l);

  // User data counts as one argument, so we need to account for that
  Foo * foo = l_CheckFoo(l, 1);

  std::cout << "-- l_Foo_call() called with " << argc - 1 << " arguments:" << std::endl;
  
  // User data is at top, so real arguments start from 2
...
int LUA_API luaopen_libfoo (lua_State *l)
{
    ...
    lua_setglobal(l, "Foo");
	luaL_openlib(l, PROJECT_MODULENAME, sFooRegs, 0);
	return 1;
}
