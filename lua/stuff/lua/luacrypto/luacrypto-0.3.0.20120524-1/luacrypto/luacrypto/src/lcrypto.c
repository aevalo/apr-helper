...
static int pkey_to_pem(lua_State *L)
{
  ...
  int private_ = lua_isboolean(L, 2) && lua_toboolean(L, 2);
  ...
  if (private_ && pkey_st->type == EVP_PKEY_DSA)
    ret = PEM_write_bio_DSAPrivateKey(mem, pkey_st->pkey.dsa, NULL, NULL, 0, NULL, NULL);
  else if (private_ && pkey_st->type == EVP_PKEY_RSA)
...
{
  return (struct x509_cert *)luaL_checkudata(L, 1, LUACRYPTO_X509_CERT_NAME);
}

static int x509_cert_fnew(lua_State *L)
{
  struct x509_cert *x = (struct x509_cert *)lua_newuserdata(L, sizeof(struct x509_cert));
...
{
  struct x509_ca *x = (struct x509_ca *)lua_newuserdata(L, sizeof(struct x509_ca));
...
{
  return (struct x509_ca *)luaL_checkudata(L, 1, LUACRYPTO_X509_CA_NAME);
}
